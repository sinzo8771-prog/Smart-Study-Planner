...
export async function GET(request: NextRequest) {
  try {
    ...
    const systemPrompt = `You are an expert study advisor. Provide personalized, actionable study tips based on the student's progress.`;
    
    const userMessage = `Based on this student's progress, provide 3-5 personalized study tips and recommendations.
...
Provide actionable, encouraging study tips formatted as markdown bullet points.`;
    ...
    const tips = completion.choices?.[0]?.message?.content || getDefaultTips();
