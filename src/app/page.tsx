...
const UserManagement = ({ user: _user }: UserManagementProps) => {
  ...
  const handleBulkDelete = async () => {
    try {
      setIsSubmitting(true);
      await Promise.all(selectedUsers.map(id => api.delete(`/api/users?id=${id}`)));
  ...
  const handleExportUsers = () => {
    const headers = ['Name', 'Email', 'Role', 'Created At', 'Subjects', 'Tasks', 'Quiz Score'];
    const rows = users.map(u => [
      u.name,
      u.email,
      u.role,
      u.createdAt ? formatDate(u.createdAt) : '-',
      String(u._count?.subjects || 0),
      String(u._count?.tasks || 0),
      Math.round(u.stats?.averageQuizScore || 0) + '%',
    ]);
  ...
  const quickActions = [
    { icon: UserPlus, label: 'Add User', description: 'Create new account', color: 'from-blue-500 to-blue-600', action: () => { setFormData({ name: '', email: '', password: '', role: 'student' }); setShowAddUserDialog(true); } },
    { icon: Download, label: 'Export Data', description: 'Download user list', color: 'from-green-500 to-green-600', action: handleExportUsers },
    { icon: Brain, label: 'Manage Quizzes', description: 'Create & edit quizzes', color: 'from-purple-500 to-purple-600', action: () => {} },
    { icon: BookOpen, label: 'Manage Courses', description: 'Course management', color: 'from-orange-500 to-orange-600', action: () => {} },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Manage users, content, and system settings</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-2 border-b border-gray-200 dark:border-gray-700 pb-2">
        {[
          ...
          <Button
            ...
            className={activeTab === tab.id ? 'bg-gradient-to-r from-blue-500 to-purple-600' : ''}
          >
              ...
              <Card className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-0">
                <CardContent className="p-4">
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    {Math.round((studentCount / (totalUsers || 1)) * 100)}% of users
                  </div>
                </CardContent>
              </Card>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <Card className="bg-gradient-to-br from-purple-50 to-violet-50 dark:from-purple-900/20 dark:to-violet-900/20 border-0">
                <CardContent className="p-4">
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    {Math.round((adminCount / (totalUsers || 1)) * 100)}% of users
                  </div>
                </CardContent>
              </Card>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              <Card className="bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20 border-0">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-3xl font-bold text-orange-600 dark:text-orange-400">{stats?.quizzes?.totalAttempts || 0}</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Quiz Attempts</p>
                    </div>
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    {stats?.quizzes?.passRate || 0}% pass rate
                  </div>
                </CardContent>
              </Card>
            </motion.div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {quickActions.map((action, i) => (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={action.action}
                      className="p-4 rounded-xl bg-gradient-to-br text-left transition-all hover:shadow-lg"
                    >
              ...
              <CardContent>
                <div className="space-y-3 max-h-64 overflow-y-auto">
                  {activityLog.slice(0, 5).map((activity, i) => (
                    <div key={activity.id} className="flex items-center gap-3 p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                ...
                <div className="flex flex-wrap gap-3">
                  {recentSignups.map((u) => (
                    ...
                    >
                      ...
                      <Badge variant="outline" className="ml-2 capitalize text-xs">{u.role}</Badge>
                    </div>
                  ))}
                </div>
              </CardContent>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-gray-800">
                      <tr>
                        <th className="p-4 font-medium text-sm w-10">
                          <Checkbox
                      ...
                      {users.length === 0 ? (
                        ...
                        users.map((u) => (
                          <motion.tr
                            ...
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${selectedUsers.includes(u.id) ? 'bg-blue-50 dark:bg-blue-900/10' : ''}`}
                          >
                            <td className="p-4">
                              <Checkbox
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              All their data including subjects, tasks, and quiz attempts will be permanently removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isSubmitting}>Cancel</AlertDialogCancel>
            <AlertDialogAction 
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Deleting...' : `Delete ${selectedUsers.length} Users`}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
