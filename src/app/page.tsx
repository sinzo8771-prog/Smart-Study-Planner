...
const LandingPage = ({ onLogin, onRegister }: LandingPageProps) => {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-slate-900 overflow-x-hidden">
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div
            ...
            className="fixed top-16 left-0 right-0 z-40 bg-gradient-to-r from-green-500 to-indigo-600 text-white py-3 px-4 text-center shadow-lg"
          >
      ...
      {/* Pricing */}
      <section id="pricing" className="py-12 sm:py-16 md:py-24 px-4 bg-white dark:bg-slate-900">
        <div className="max-w-6xl mx-auto">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 max-w-5xl mx-auto">
            {[ 
              ...
              { 
                ...
                price: '$9.99', 
                period: '/month', 
                features: ['Unlimited subjects', 'Full analytics dashboard', 'Quiz generator', 'Priority support', 'Course access'], 
                popular: true, 
                cta: 'Subscribe Now'
              },
              { 
                ...
                price: '$19.99', 
                period: '/month', 
                features: ['Everything in Pro', 'Study assistant', 'Custom themes', 'API access', 'Team collaboration', 'Dedicated support'], 
                cta: 'Subscribe Now'
              },
            ].map((plan, index) => (
              <motion.div
              ...
              >
                <Card className={`h-full relative ${plan.popular ? 'border-2 border-indigo-500' : 'border border-slate-200 dark:border-slate-700'} bg-white dark:bg-slate-800 overflow-hidden group`}>
                  {plan.popular && (
                    <div className="absolute -top-0 right-4">
                      <Badge className="bg-indigo-500 text-white rounded-b-lg rounded-t-none px-3 sm:px-4 py-1 text-xs sm:text-sm">
                        Most Popular
                      </Badge>
                    </div>
                  )}
                  <CardContent className="p-5 sm:p-6 md:p-8 pt-8 sm:pt-10 md:pt-12">
                    <div className="text-center mb-6 sm:mb-8">
                      <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mb-1 sm:mb-2">{plan.name}</h3>
                      <div className="flex items-end justify-center gap-1">
                        <span className="text-3xl sm:text-4xl md:text-5xl font-bold text-slate-900 dark:text-white">{plan.price}</span>
                        <span className="text-slate-500 mb-1 sm:mb-2 text-sm sm:text-base">{plan.period}</span>
                      </div>
                    </div>
                    <ul className="space-y-2 sm:space-y-3 md:space-y-4 mb-6 sm:mb-8">
                      {plan.features.map((f, i) => (
                        ...
                        >
                          <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full ${plan.popular ? 'bg-indigo-500' : 'bg-slate-400'} flex items-center justify-center flex-shrink-0`}>
